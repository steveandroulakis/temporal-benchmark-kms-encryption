...
import { benchmarkEncryptionWorkflow } from './workflows';
...
export async function runWorkflow(config: ConfigObj): Promise<String> {
  ...
  const transactionId = 'Benchmark-Encryption-' + uuidv4();

  // start() returns a WorkflowHandle that can be used to await the result
  const handle = await client.workflow.start(benchmarkEncryptionWorkflow, {
